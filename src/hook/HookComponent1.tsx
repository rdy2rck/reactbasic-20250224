...
// - useEffect : 컴포넌트의 생명주기에 따라 특정 작업을 수행할 수 있도록 하는 함수 (mount, update, unmount에 해당하는 생명 주기에 따라 실행)
// - useRef : 특정 데이터를 기억하기 위한 함수 값이 변경 되더라도 렌더링을 수행하고 싶지 않을 때, DOM 요소를 기억하고 싶을 때 사용
// - useContext : 컴포넌트 트리 상에서 전역 상태를 공유하고자 할 때 사용하는 함수
// - useReducer : 상태와 관련된 로직을 관리하는 함수 (상태 업데이트 함수를 컴포넌트 외부로 옮길 수 있도록 하여 코드 관리를 수월하게 도와줌)
...
export default function HookComponent1() {
  ...
  // 스코프할 배열
  // useEffect(() => {
  //   console.log('두 번째 매개변수를 전달하지 않은 effect');
  //   setState(state + 1);
  // });
  ...
  useEffect(() => {
    console.log('state 혹은 show 값이 변경됨!');
  }, [state, show]);
