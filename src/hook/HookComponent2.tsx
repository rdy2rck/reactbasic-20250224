...
export default function HookComponent2() {
  ...
  // 이땐, 해당 요소 시작 태그의 ref 속성에 해당 참조변수를 지정해야 함
  ...
  const onFirstKeydownHandler = (event: KeyboardEvent<HTMLInputElement>) => {
    ...
    // react에서는 document.getElementXXX 사용을 지양하는 게 좋음
    // - 실제 HTML이 렌더링 되기 전 코드를 읽는다면 요소를 찾을 수 없음
    ...
    // 단, 가급적이면 사용을 안 하는 게 좋음 -> 호출 전에 상단에서 검증 (if문 이용)을 먼저 하는 것이 권장됨
