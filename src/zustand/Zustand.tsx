...
// - store : 관리할 상태들과 각 상태의 변경 함수들을 집합
// - create 함수를 호출하면 store를 반환하는 훅 함수를 반환
// - Typescript의 경우 create 함수에 store의 구조를 제너릭으로 전달해야 함
...
const useStore = create<Store>((set) => ({
  state1: 0,
  // create 함수의 매개변수로 전달된 콜백 함수의 set 함수를 통해서 상태를 변경해야 함
  // set 함수의 매개변수로는 현재 상태(store)를 받는 콜백 함수를 전달
  // set 함수의 매개변수로 전달한 콜백 함수는 변경된 상태 객체(store)를 반환
  setState1: (state1: number) => set((state) => {return {...state, state1};}),
  increaseState1: () =>
